#!/usr/bin/env Rscript
# Title: TCGA RNA-seq retrieval by explicit barcode list
# Description: Given a text file of TCGA barcodes (first 16 characters used as
#   sample IDs), this helper script downloads count matrices and metadata from
#   GDC for a specified project/workflow, chunking queries to avoid API limits.
#   It also safeguards against missing clinical annotations by wrapping
#   colDataPrepare. Outputs are flattened TSVs that downstream DE scripts can
#   consume without further pre-processing.
...
prepare_with_fallback <- function(query) {
  # Guard against missing clinical metadata by temporarily overriding colDataPrepare
...
chunk_size <- if (length(args) >= 5) as.integer(args[[5]]) else 50 # limits per-query sample count for reliability
workflow_type <- if (length(args) >= 6) args[[6]] else "HTSeq - Counts" # pass "STAR - Counts" to align with multi-project script
# 1) Parse arguments describing barcode file, project, output paths, chunking, and workflow

if (!file.exists(barcode_file)) {
  stop(sprintf("Barcode file not found: %s", barcode_file))
}

# 2) Read and sanitize barcode list
barcodes <- trimws(readLines(barcode_file, warn = FALSE)) # accept whitespace-separated files from earlier steps
barcodes <- barcodes[nchar(barcodes) > 0]
if (!length(barcodes)) {
  stop("No barcodes detected in input file.")
}

sample_ids <- unique(substr(barcodes, 1, 16)) # collapse replicate aliquots to sample level
message(sprintf("Detected %d unique sample IDs", length(sample_ids)))

# 3) Collapse barcodes to unique sample IDs and prepare chunking
chunks <- split(sample_ids, ceiling(seq_along(sample_ids) / chunk_size)) # throttle API usage
...
fetch_chunk <- function(ids, idx, total) {
  # Run query/download/prepare for a chunk; fall back to minimal metadata when needed
...
# 4) Helper to query, download, and prepare one chunk of samples
...
assay_list <- lapply(se_list, SummarizedExperiment::assay) # convert SE objects into matrices
...
counts_mat <- do.call(cbind, assay_list) # reconstruct a single expression matrix from chunked SE objects
if (is.null(rownames(counts_mat))) {
  stop("Counts matrix is missing gene identifiers.")
}

# 5) Combine individual SummarizedExperiment assays into one matrix
...
coldata_dt[, ord := match(sample_id, colnames(counts_mat))] # keep metadata aligned with counts columns
setorder(coldata_dt, ord)
coldata_dt[, ord := NULL]
setcolorder(coldata_dt, c("sample_id", setdiff(names(coldata_dt), "sample_id")))

# 6) Concatenate associated metadata (colData) for reference
flatten_lists <- function(dt) {
  # SummarizedExperiment metadata columns can be nested lists; flatten for TSV output
...
# 7) Flatten nested metadata columns to plain character values
...
fwrite(counts_dt, counts_out, sep = "\t") # matrix used by downstream DESeq2 workflow
fwrite(coldata_dt, metadata_out, sep = "\t") # companion metadata for reproducibility/QC
# 8) Persist counts and metadata tables for downstream use
